...
export function PlantMapViewer(props: {
  ...
  const { map, selectedAsset, allAssets, showAllMarkers, addingMarker, onAddMarker } = props;
      ...
      >
        ...
        {markers.map((m) => (
          <div
            key={m.id}
            className={
              'absolute -translate-x-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-primary-600 ring-2 ring-white dark:ring-gray-900'
            }
